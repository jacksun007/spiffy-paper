...
and the correctness of Spiffy libraries. We then evaluate the performance 
...
\subsection{Corruption Experiment\label{subsec:corruption_experiment}}

To evaluate the correctness of Spiffy generated libaries, we ran a series 
of type-specific corruption experiments on existing file system 
dump tools and the Spiffy dump tool. The type-specific corruption experiment
takes a 128MB file system image filled with 12000 tiny files (less than 4 bytes) 
and some directories, then corrupts a specific field of a selected
instance of a specified type (e.g., one of the inode structures). The field 
is either corrupted to a random value or zero.

The Spiffy dump tool was able to generate correctly formatted XML files in the 
face of arbitrary single-field corruptions. When corruption is detected during
the parsing of a container or pointer fetch (i.e., pointer address out-of-bound 
or fails placement constraint), an error is printed and the program stops the
traversal process. 

We also run the same corrupted images on existing tools.
For dumpe2fs (dump tool for Ext4) v1.42.13, we recorded a single crash when 
the \texttt{s\_creator\_os}
field of the super block is corrupted. For dump.f2fs v1.6.1-1, we observed 6 
instances
of abnormal program termination, including assertion errors and segmentation
faults. 4 of the crashes were due to corruption in the super block, and 1 crash
each was detected for the checkpoint and inode structure. We were unable
to trigger any crash-related bugs in btrfs-debug-tree v4.4.

The results were expected since F2FS is a relatively young file system. Btrfs
applies metadata checksumming technique to avoid metadata
corruption, and thus requires correlated corruption where a metadata field 
is corrupted and somehow the metadata block passes checksum verification.
Lastly, Ext4, being the most 
battle-tested file system for Linux, has hardened itself against various forms
of corruption. It is encouraging to know that Spiffy is still able to do better 
than dumpe2fs in terms of robustness without requiring maturity or
