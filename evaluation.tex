...
dumpe2fs (dump tool for Ext4) 
and btrfs-debug-tree were also able terminate gracefully when attempting to
parse the same corrupted image. However, we recorded dump.f2fs 10 instances
of abnormal program termination, including assertion errors and segmentation
faults. 8 of the crashes were due to corruption in the super block, and 1 crash
was detected for the checkpoint and inode structure.
...
corruption not caused by the file system itself, and thus requires correlated
corruption to reveal any potential bugs. Lastly, Ext4, being the most 
battle-tested file system for Linux, has hardened itself against various forms
of corruption. It is encouraging to know that Spiffy is able to do as well as 
dumpe2fs in terms of robustness without requiring significant testing efforts.
