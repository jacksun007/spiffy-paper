...
\noindent\textbf{Free Space Tool:} The file system free space tool has 271 lines of file-system independent C++ code. In addition, 76 lines are Ext4 specific, 77 lines are Btrfs specific, and 194 lines are F2FS specific. F2FS requires more code due to the complex format of its block allocation information.
...
We also run the same corrupted images on existing tools. For dumpe2fs (dump tool for Ext4) v1.42.13, we found a single crash when the \texttt{s\_creator\_os} field of the super block is corrupted. For dump.f2fs v1.6.1-1, we observed 6 instances of abnormal program termination, including assertion errors and segmentation faults. Four of the crashes were due to corruption in the super block, and one crash each was detected for the checkpoint and inode structures. We were unable to trigger any crash-related bugs in btrfs-debug-tree v4.4.

These results are expected since F2FS is a relatively young file system. Btrfs applies metadata checksumming technique to avoid metadata corruption, and thus requires correlated corruption where a metadata field is corrupted and somehow the metadata block passes checksum verification.  Lastly, Ext4, being the most battle-tested file system for Linux, has hardened itself against various forms of corruption. It is encouraging that a Spiffy application is more robust than dumpe2fs without requiring maturity or significant testing effort.

Incidentally, we also tried an extensive set of random corruption experiments, and none of the existing tools had crashes, showing that our type-specific corruptor is a useful tool for testing the robustness of these applications.
