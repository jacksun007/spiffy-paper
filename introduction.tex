...
We compile the annotated sources to generate a Spiffy library that provides interfaces for type-safe parsing, traversal and update of file system metadata. The library allows a developer to write actions for different pieces of file system metadata, invoking file-system specific or generic code as needed, for their offline or online application. {\color{red}We support online applications that need to read metadata, such as differentiated storage services~\cite{Mesnier2011}, but not ones that need to modify metadata such as online defragmentation.}
