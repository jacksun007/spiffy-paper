...
\hline 
{\small{}File system checker~\cite{Gunawi08b}} & {\small{}offline} & \multirow{4}{*}{{\small{}reliability}}\tabularnewline
\cline{1-2} 
...
\hline 
{\small{}File system conversion tool} & {\small{}either} & \multirow{2}{*}{{\small{}administrative}}\tabularnewline
\cline{1-2} 
...
We expand on how Spiffy addresses each of these points in~\ref{subsec:Annotation-Design}. Existing specification tools do not handle all of these concerns, as discussed in~\ref{sec:Related_Work}.
...
The rest of the paper is organized as follows. In \ref{sec:Extended-Motivation}, we motivate the need for our approach. \ref{sec:Approach} presents the core concepts that led to the design of the annotation language and the library API. \ref{sec:File-System-Applications} describes the applications that we have implemented using the generated library. \ref{sec:Implementation} describes the implementation of our system, and \ref{sec:Evaluation} assesses the programming effort needed to annotate each file system and the performance of our applications. We present related work in \ref{sec:Related_Work} and discuss our conclusions in \ref{sec:Conclusion}.  For reference, Appendix~\ref{sec:Annotation_Language} shows our file system annotation language with examples of annotated structures for the Ext4, Btrfs and F2FS file systems. 