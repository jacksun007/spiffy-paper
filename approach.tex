...
The purpose of our file system annotation language is to enable type-safe interpretation of file system structures, in both offline and online contexts.
...
The \texttt{Entity} base class provides a common interface for all metadata structures and their fields. The \texttt{process\_pointers} function invokes the \texttt{visit} function of an application-defined Visitor class on each pointer within the entity. The \texttt{process\_by\_type} function allows visiting a specific type of structure that is reachable from the entity. Unlike the other two process functions, \texttt{process\_by\_type} will automatically follow pointers. Thus, for example, invoking \texttt{process\_by\_type} on the super block with the inode structure as an argument will result in visiting all inodes in the file system.
...
The correctness of Spiffy applications depends on correctly written annotations. Therefore, if and when file system format changes do occur, the specifications will need to be updated. In addition, Spiffy applications will also need to update all file-system specific code that is affected by the format changes. These changes will likely only affect code that directly operates on the updated metadata structures, since the Spiffy library will take care of safe traversal and parsing of any intermediate structures.
