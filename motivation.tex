...
\item The e2fsck file system checker failed to detect corrupted directory entries if the size field of the entries was set to zero, which results in no repair being performed. Ironically, other programs, such as debugfs, ls, and the file system itself, can correctly detect the corruption.

\item Ext2/3/4 inodes contain union fields for storing operating system specific metadata. A sanity check was omitted in e2fsck prior to accessing this field, and repairs were always performed assuming that the creator OS is Linux. Consequently, the file system becomes corrupt for Hurd and possibly other operating systems.`
